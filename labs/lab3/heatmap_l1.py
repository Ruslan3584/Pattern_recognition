...
def main():
    parser = argparse.ArgumentParser(
        description="Minimize the risk of the Bayesian strategy for L1 loss function."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Delta value for loss function.",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    cumsum = np.cumsum(np.insert(heatmap_norm, 0, 0))
    i = np.arange(0, args.n - args.delta + 1)
    left = np.maximum(0, i - args.delta)
    right = i + args.delta
    res = np.argmax(cumsum[right] - cumsum[left])
